var stops = [{
	name: 'Lillogata',
	description: 'Start', 
	latlng: [59.942980, 10.767825],
	icon: 'circle-stroked',
	color: '#145291',
	size: 'l'
},{
	name: '<a href="http://ut.no/hytte/sinnerdammen">Sinnerdammen</a>',
	description: 'Ubetjent', 
	latlng: [60.169, 10.496],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: 'Jevnaker',
	description: 'Butikker', 
	latlng: [60.241, 10.372],
	icon: 'grocery',
	color: '#145291',
	size: 's'
},{
	name: 'Hvalstjern',
	description: 'Telt', 
	latlng: [60.275, 10.233],
	icon: 'campsite',
	color: '#145291',
	size: 'm'
},{
	name: 'Hallingby',
	description: 'Butikk', 
	latlng: [60.271,10.185],
	icon: 'grocery',
	color: '#145291',
	size: 's'
},{
	name: '<a href="http://ut.no/hytte/tjuvenborgkoia">Tjuenborgkoia</a>',
	description: 'Ubetjent',
	latlng: [60.339, 9.977],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: '<a href="http://ut.no/hytte/storekrakkoia">Storekrakkoia</a>',
	description: 'Ubetjent',
	latlng: [60.497, 9.711],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: '<a href="http://ut.no/hytte/f%C3%B8nhuskoia">Fønhuskoia</a>',
	description: 'Ubetjent',
	latlng: [60.61, 9.432],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: 'Valdreslihovda',
	description: 'Telt', 
	latlng: [60.676, 9.269],
	icon: 'campsite',
	color: '#145291',
	size: 'm'
},{
	name: 'Golsfjellet',
	description: '<a href="http://www.kamben.no/">Kamben Høyfjellshotell</a>',
	latlng: [60.802, 8.98],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: 'Robru',
	description: '<a href="http://www.joker.no/Butikkene/Buskerud/Joker-Robru/">Butikk</a>', 
	latlng: [60.747,8.869],
	icon: 'grocery',
	color: '#145291',
	size: 's'
},{
	name: 'Buhovdvatnet',
	description: 'Telt', 
	latlng: [60.742, 8.675],
	icon: 'campsite',
	color: '#145291',
	size: 'm'
},{
	name: '<a href="http://www.bergsjostolen.no">Bergsjøstølen</a>',
	description: 'Fjellstove', 
	latlng: [60.716, 8.291],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: '<a href="http://ut.no/hytte/iungsdalshytta">Iungsdalshytta</a>',
	description: 'Betjent', 
	latlng: [60.81, 7.934],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: '<a href="http://ut.no/hytte/kongshelleren">Kongshelleren</a>',
	description: 'Selvbetjent', 
	latlng: [60.777, 7.701],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: '<a href="http://ut.no/hytte/geiterygghytta">Geiterygghytta</a>',
	description: 'Betjent', 
	latlng: [60.701, 7.604],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: '<a href="http://ut.no/hytte/hallingskeid">Hallingskeid</a>',
	description: 'Selvbetjent', 
	latlng: [60.668, 7.248],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: '<a href="http://ut.no/hytte/kaldavasshytta">Kaldavasshytta</a>',
	description: 'Ubetjent', 
	latlng: [60.702, 7.08],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: 'Mjølfjell',
	description: '<a href="http://www.mjolfjell.no">Fjellstove</a>', 
	latlng: [60.703, 6.937],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: 'Mjølfjell',
	description: 'Butikk', 
	latlng: [60.693, 6.843],
	icon: 'grocery',
	color: '#145291',
	size: 's'
},{
	name: 'Raundalsryggen',
	description: 'Telt', 
	latlng: [60.637, 6.712],
	icon: 'campsite',
	color: '#145291',
	size: 'm'
},{
	name: 'Flatlandsmo',
	description: 'Kiosk', 
	latlng: [60.594, 6.583],
	icon: 'cafe',
	color: '#145291',
	size: 's'
},{
	name: 'Bulko',
	description: 'Privat hytte', 
	latlng: [60.555, 6.613],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: '<a href="http://www.rongastovo.no">Rongastovo</a>',
	description: 'Fjellstove', 
	latlng: [60.515, 6.478],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: '<a href="http://ut.no/hytte/torfinnsheim">Torfinnsheim</a>',
	description: 'Ubetjent', 
	latlng: [60.562, 6.243],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: '<a href="http://ut.no/hytte/vending-turisthytte">Vending</a>',
	description: 'Selvbetjent', 
	latlng: [60.474, 6.075],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: '<a href="http://www.bergenoghordalandturlag.no/article.php?ar_id=42087&fo_id=198">Gullhorgabu</a>',
	description: 'Selvbetjent', 
	latlng: [60.5011, 5.9533],
	icon: 'building',
	color: '#145291',
	size: 'm'
},{
	name: 'Holmavatnet',
	description: 'Telt', 
	latlng: [60.425, 5.682],
	icon: 'campsite',
	color: '#145291',
	size: 'm'
},{
	name: 'Nordåsgrenda',
	description: 'Mål', 
	latlng: [60.309, 5.323],
	icon: 'bar',
	color: '#145291',
	size: 'l'
}];

var map = L.map('map').fitBounds([[59.92,5.19],[60.72,10.73]]);

L.tileLayer('http://opencache.statkart.no/gatekeeper/gk/gk.open_gmaps?layers=norges_grunnkart&zoom={z}&x={x}&y={y}', {
	attribution: '&copy; <a href="http://osm.org/copyright">Kartverket</a>'
}).addTo(map);

for (var i = 0; i < stops.length; i++) { 
	var stop = stops[i],
	    icon = L.MakiMarkers.icon({icon: stop.icon, color: stop.color, size: stop.size});

	L.marker(stop.latlng, {icon: icon}).addTo(map).bindPopup('<strong>' + stop.name + '</strong><br>' + stop.description);
}
